...
def pull_sports_schedules(month=None) -> None:
    ...
    games = [[e.strip() for e in line.strip().split(",")] for line in r.text.split("\n")[1:] if not ("Cancelled" in line or "Changed" in line)]
    for g in games:
        try:
            Event.objects.get_or_create(
                title=g[0].split("(")[0].strip(),
                description=g[5][g[5].index("Opponent:") + 10:],
                time=timezone.make_aware(datetime.strptime("{} {}".format(g[1], g[2]), "%m/%d/%Y %I:%M%p")),
                location=g[6],
